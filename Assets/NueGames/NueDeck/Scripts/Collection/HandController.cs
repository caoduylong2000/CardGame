...
namespace NueGames.NueDeck.Scripts.Collection
{
    public class HandController : MonoBehaviour
    {
        ...
        [SerializeField] private Vector3 curveStart;
        [SerializeField] private Vector3 peakPoint;
        [SerializeField] private Vector3 curveEnd;
        ...
        private void InitHand()
        {
            ...
            _b = transform.TransformPoint(peakPoint);
        ...
        {
            bool anyCardHoveredOrDragged = false;

            // Vòng lặp đầu tiên để xử lý từng thẻ
            for (var i = 0; i < count; i++)
            {
                ...
                if (SceneManager.GetActiveScene().buildIndex == GameManager.SceneData.combatSceneIndex)
                {
                    DetectCardType(card, CardActionType.AttackByCardType, CardType.Attack);
                }
                ...
                var mouseHoveringOnSelected = onSelectedCard && mouseCloseToCard && _mouseInsideHand;
            ...
            anyCardHoveredOrDragged = false;
            for (var i = 0; i < count; i++)
            {
                var card = hand[i];
                var cardTransform = card.transform;

                var noCardHeld = _heldCard == null;
                var onSelectedCard = noCardHeld && _selected == i;
                var onDraggedCard = noCardHeld && _dragged == i;
                ...
                {
                    anyCardHoveredOrDragged = true;
                    if (instantiatedCard == null)
                    {
                        if (SceneManager.GetActiveScene().buildIndex == GameManager.SceneData.combatSceneIndex)
                            instantiatedCard = Instantiate(CollectionManager.cardPopUp, UIManager.CombatCanvas.transform);
                        else if (SceneManager.GetActiveScene().buildIndex == GameManager.SceneData.tutorialSceneIndex)
                            instantiatedCard = Instantiate(TutorialCollectionManager.cardPopUp, UIManager.CombatCanvas.transform);

                        instantiatedCard.transform.localScale = new Vector3(1.6f, 1.6f, 1);
                    }
                    instantiatedCard.GetComponent<CardUI>().SetCard(card.CardData, false);

                    instantiatedCard.GetComponentInChildren<AutoScrollDesc>().ActiveScroll();

                    break;
                }
            }

            if (!anyCardHoveredOrDragged && instantiatedCard != null)
            {
        ...
        private void DetectCardType(CardBase card, CardActionType cardActionType, CardType cardType)
        {
            // Check if there is an element in CardActionDataList with CardActionType.AttackByCardType
            bool hasAttackByCardTypeAction = card.CardData.CardActionDataList
                                               .Any(cardAction => cardAction.CardActionType == cardActionType);

            if (hasAttackByCardTypeAction)
            {
                // Count the number of elements whose CardActionType
                //int cardTypeCount = CollectionManager.Instance.HandPile
                //            .Count(handCard => handCard.CardActionDataList
                //                .Any(cardAction => cardAction.CardActionType == cardActionType));

                int cardTypeCount = CollectionManager.Instance.HandPile
                                            .Count(handCard => handCard.CardType == cardType);

                // Compare cardTypeCount with the count in HandPile
                bool shouldSetInactive = cardTypeCount != CollectionManager.Instance.HandPile.Count;

                // Call SetInactiveMaterialState if the condition is satisfied
                card.SetInactiveMaterialState(shouldSetInactive);
            }
        }
        ...
        private void HandleDraggedCardOutsideHand(bool mouseButton, Vector2 mousePos)
        {
            if (_heldCard != null)
            {
                ...
                {
                    TutorialCombatManager.HighlightCardTarget(_heldCard.CardData.CardActionDataList[0].ActionTargetType);
                }
                else
                {
                ...
                if (!GameManager.PersistentGameplayData.CanSelectCards || _mouseInsideHand)
                {
                    ...
                    if (SceneManager.GetActiveScene().buildIndex == GameManager.SceneData.tutorialSceneIndex)
                    {
                        TutorialCombatManager.DeactivateCardHighlights();
                    }
                    else
                    {
        ...
        private void PlayCard(Vector2 mousePos)
        {
            ...
            if (GameManager.PersistentGameplayData.CanUseCards && GameManager.PersistentGameplayData.CurrentMana >= _heldCard.CardData.ManaCost)
            {
                ...
                var _canUse = false;
                CharacterBase selfCharacter;

                if (SceneManager.GetActiveScene().buildIndex == GameManager.SceneData.tutorialSceneIndex)
                {
                    selfCharacter = TutorialCombatManager.CurrentMainAlly;
                }
                else
                {
        ...
        private void OnDrawGizmos()
        {
            ...
            Gizmos.DrawSphere(peakPoint, 0.03f);
            ...
            {
                float t = (i + 1) / 20f;
                Vector3 p2 = GetCurvePoint(curveStart, peakPoint, curveEnd, t);
