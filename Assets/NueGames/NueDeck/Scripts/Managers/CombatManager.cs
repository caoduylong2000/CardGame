...
namespace NueGames.NueDeck.Scripts.Managers
{
    public class CombatManager : MonoBehaviour
    {
        ...
        [Header("Crystal drop rate")]
        public float enemyDropRate;
        public float eliteDropRate;
        public float bossDropRate;
        ...
        private void Start()
        {
            ...
            UIManager.CombatCanvas.CombatWinPanel.SetActive(false);
        ...
        public void WinCombat()
        {
            ...
            float crystalDropRate;

            if (CurrentEnemiesList[0].tag == "Enemy")
            {
                crystalDropRate = enemyDropRate * GameManager.CrystalRate;
            }
            else if (CurrentEnemiesList[0].tag == "Elite")
            {
                crystalDropRate = eliteDropRate * GameManager.CrystalRate;
            }
            else if (CurrentEnemiesList[0].tag == "Boss")
            {
                crystalDropRate = bossDropRate * GameManager.CrystalRate;
            }
            else return;

            var successValue = UnityEngine.Random.Range(0f, 1f);

            if (successValue <= crystalDropRate)
            {
                if (CurrentEnemiesList[0].tag == "Boss")
                    UIManager.RewardCanvas.BuildReward(RewardType.BossCrystal);
                else
                    UIManager.RewardCanvas.BuildReward(RewardType.Crystal);
            }

            UIManager.RewardCanvas.BuildReward(RewardType.ItemSellGold);
