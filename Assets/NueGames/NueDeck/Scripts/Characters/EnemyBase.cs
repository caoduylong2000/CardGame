...
namespace NueGames.NueDeck.Scripts.Characters
{
    public class EnemyBase : CharacterBase, IEnemy
    {
        ...
        public override void BuildCharacter()
        {
            ...
            var bonusStats = GameManager.Instance.PersistentGameplayData.CurrentStageId + 1;

            CharacterStats = new CharacterStats((int)(EnemyCharacterData.MaxHealth * bonusStats),EnemyCanvas);
            CharacterStats.OnDeath += OnDeath;
            CharacterStats.OnHealthChanged += UIManager.Instance.CombatCanvas.UpdateSuccessText;
            CharacterStats.OnHealthChanged += UIManager.Instance.CombatCanvas.UpdateSuccessSlider;
            CharacterStats.SetCurrentHealth(CharacterStats.CurrentHealth);

            if(SceneManager.GetActiveScene().buildIndex == GameManager.SceneData.tutorialSceneIndex)
            {
                TutorialCombatManager.OnAllyTurnStarted += ShowNextAbility;
                TutorialCombatManager.OnEnemyTurnStarted += CharacterStats.TriggerAllStatus;
            }    
        ...
        protected override void OnDeath()
        {
            ...
            if (SceneManager.GetActiveScene().buildIndex == GameManager.SceneData.tutorialSceneIndex)
            {
                TutorialCombatManager.OnEnemyDeath(this);
            }
            else
            {
        ...
        protected virtual IEnumerator AttackRoutine(EnemyAbilityData targetAbility)
        {
            ...
            if (SceneManager.GetActiveScene().buildIndex == GameManager.SceneData.tutorialSceneIndex)
            {
                if (TutorialCombatManager == null) yield break;

                var target = TutorialCombatManager.CurrentAlliesList.RandomItem();
        ...
        protected virtual IEnumerator BuffRoutine(EnemyAbilityData targetAbility)
        {
            ...
            if (SceneManager.GetActiveScene().buildIndex == GameManager.SceneData.tutorialSceneIndex)
            {
                var target = TutorialCombatManager.CurrentEnemiesList.RandomItem();
