...
namespace NueGames.NueDeck.Scripts.Characters
{
    ...
    { 
        ...
        private void SetAllStatus()
        {
            ...
            StatusDict[StatusType.Vulnerable].OnTriggerAction += CheckVulnerableStatus;

            StatusDict[StatusType.Weak].DecreaseOverTurn = true;
            StatusDict[StatusType.Weak].OnTriggerAction += CheckWeakStatus;

            StatusDict[StatusType.Frail].DecreaseOverTurn = true;
            StatusDict[StatusType.Frail].OnTriggerAction += CheckFrailStatus;
        ...
        public void UpdateData(int currentHealth, int maxHealth)
        {
            ...
            maxHealth = MaxHealth;
            //Set
            GameManager.PersistentGameplayData.AllyHealthDataList[0].CurrentHealth = currentHealth;
            GameManager.PersistentGameplayData.AllyHealthDataList[0].MaxHealth = maxHealth;
        }

        public void AllyHeal(int value)
        {
            for (int i = 0; i < value; i++)
            {
                MaxHealth = (int)(GameManager.PersistentGameplayData.AllyList[0].AllyCharacterData.MaxHealth * GameManager.PlayerHP);
                CurrentHealth++;
                ...
                UIManager.InformationCanvas.UpdateHealStats(CurrentHealth, MaxHealth);
            }
            OnHealthChanged?.Invoke(CurrentHealth,MaxHealth);
        }
        ...
        public void EnemyHeal(int value)
        {
            for (int i = 0; i < value; i++)
            {
                MaxHealth = CombatManager.Instance.CurrentMainEnemy.CharacterStats.MaxHealth;
                CurrentHealth++;
        ...
        public void CheckWeakStatus()
        {
            if (StatusDict[StatusType.Weak].StatusValue <= 0)
            {
                IsWeak = false;
                return;
            }
        ...
        public void CheckFrailStatus()
        {
            if (StatusDict[StatusType.Frail].StatusValue <= 0)
            {
                IsFrail = false;
                return;
            }
